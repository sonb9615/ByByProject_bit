...
import kr.co.byby.service.ReplySymptomService;   

@RestController
@RequestMapping(value = "/symboard/restBoard")
public class SymptomReplyRestController {
   
   @Autowired
   private ReplySymptomService service;
   
   @RequestMapping(value = "/getReqlyList", method = RequestMethod.POST)
   public List<ReplySymptomVO> getReplyList(@RequestParam("no") int no) throws Exception {
      return service.getReplyList(no);
   }
   
   @RequestMapping(value = "/saveReqly", method = RequestMethod.POST)
   public Map<String, Object> saveReply(@RequestBody ReplySymptomVO replyVO) throws Exception {
   ...
   public ResponseEntity<String> replyUpdate(@PathVariable("commentno") Integer commentno, @RequestBody ReplySymptomVO replyVO){
      ResponseEntity<String> entity = null;
      try {
