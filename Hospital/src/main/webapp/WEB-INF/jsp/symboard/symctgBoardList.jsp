   ...
   $.ajax({
      ...
      success: function(result) {
         ...
         for (var i = 0; i < myObj.length; i++) {
            var item = myObj[i];
            $("#divCtg").append("<li id='ctg'><a href=/symCategory/" + item.ctg_name + ">" + item.ctg_name + "</a></li>");
         }
      },
		...
		<div align="center" id="hplist">
			...
			<div class='board-tab-outer'>
				<ul class='board-tab  w6-2 tabCategory'>
					<div id="divCtg"></div>
					<li id="total"><a
						href="${pageContext.request.contextPath}/symptomboard" id="end">전체</a></li>
				</ul>
			</div>
			<br>
			<table id="list" class="table">
				...
				<c:forEach items="${ ctgBoard }" var="symptom">
					<tr>
						...
						<td><a href="#" onclick="doAction2(${ symptom.no })"
							id="showdetail"> ${ symptom.title } </a></td>
		...
		<div id="pageNum">
			<ul class="btn-group pagination">
				<c:if test="${pageMaker.prev }">
					<li><a
						href="${ pageContext.request.contextPath }/symCategory/${ ctg_name }${ pageMaker.makeQuery(pageMaker.startPage - 1) }"><i
							class="fa fa-chevron-left"></i>이전</a></li>
				</c:if>
				<c:forEach begin="${ pageMaker.startPage }"
					end="${ pageMaker.endPage }" var="idx">
					<li><a
						href="${ pageContext.request.contextPath }/symCategory/${ ctg_name }${ pageMaker.makeQuery(idx) }"><i
							class="fa">${ idx }</i></a></li>
				</c:forEach>
				<c:if test="${ pageMaker.next && pageMaker.endPage>0 }">
					<li><a
						href="${ pageContext.request.contextPath }/symCategory/${ ctg_name }${ pageMaker.makeQuery(pageMaker.endPage+1) }"><i
